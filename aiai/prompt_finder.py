...
class PromptFragment(BaseModel):
    ...
    function_name: str = Field(description="Name of the function where the fragment was found")
    ...
    fragment_code: str = Field(description="The exact code that defines or constructs the prompt fragment")


class PromptAnalysis(BaseModel):
    """Analysis of where a prompt is constructed in the codebase."""
    fragments: List[PromptFragment] = Field(description="Code fragments that make up the prompt")
    composite_locations: List[str] = Field(description="Functions that assemble multiple fragments")


def analyze_prompt_with_llm(prompt_text: str, function_info_list: List[Any]) -> PromptAnalysis:
    """
...
@app.command()
def find_prompts(log_id: Optional[int] = typer.Option(None, help="Specific AgentRunLog ID to analyze")):
    """
    ...
    for log in logs:
        ...
        prompt = None
        if log.input_data and 'prompt' in log.input_data and isinstance(log.input_data['prompt'], str):
