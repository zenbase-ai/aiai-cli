...
def test_find_file_references(test_directory):
    ...
    assert json_references[0]['function'].name == "load_prompt", f"Expected reference in load_prompt, found in {json_references[0]['function'].name}"
    ...
    assert yaml_references[0]['function'].name == "process_config", f"Expected reference in process_config, found in {yaml_references[0]['function'].name}"
...
def test_save_data_files_to_db(test_directory):
    ...
    assert json_file.referenced_by.count() == 1, f"Expected 1 reference to JSON file, found {json_file.referenced_by.count()}"
    assert json_file.referenced_by.first().name == "load_prompt", f"Expected reference from load_prompt, found {json_file.referenced_by.first().name}"
    ...
    assert yaml_file.referenced_by.count() == 1, f"Expected 1 reference to YAML file, found {yaml_file.referenced_by.count()}"
    assert yaml_file.referenced_by.first().name == "process_config", f"Expected reference from process_config, found {yaml_file.referenced_by.first().name}"
...
def test_data_file_analyzer(test_directory):
    ...
    assert DataFileAnalysis.objects.count() == 2, f"Expected 2 analysis records, found {DataFileAnalysis.objects.count()}"
...
def test_save_data_files_to_db_crewai(test_directory):
    ...
    # Now find and save data files
    file_references = analyzer.find_and_save_data_files("/Users/amir/workspace/aiai/aiai/examples/crewai")
